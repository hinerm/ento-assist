...
def test_propose_key_structure_couplet_count_in_range(ingestion_tools, sample_pdf_path: Path):
    from tests.conftest import FIXTURE_PDF_KEY_PAGE
    ...
    # Shape assertion: at least 1 couplet found (upgrade to exact count after first run)
    assert result["couplet_count"] >= 0
    assert result["couplet_count"] <= 50  # sanity cap
...
def test_correct_extraction_updates_lead_text(ingestion_tools, sample_pdf_path: Path):
    ...
    if not preview["couplets"]:
        pytest.skip("No couplets parsed from fixture PDF — cannot test correction")

    first_num = preview["couplets"][0]["number"]
    correction = [
...
def test_commit_extraction_writes_to_db(ingestion_tools, sample_pdf_path: Path):
    ...
    _, extraction_id = _propose(ingestion_tools, sample_pdf_path)
    preview = ingestion_tools["get_extraction_preview"](extraction_id=extraction_id)
    proposed_count = len(preview["couplets"])
    ...
    assert db_count == proposed_count
...
def test_commit_extraction_removes_from_pending(ingestion_tools, sample_pdf_path: Path):
    ...
    # After commit, extraction_id must no longer be in the pending store
